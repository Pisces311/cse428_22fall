...
int main(int argc, const char **argv) {
    ...
    if ((gameName == "Pinochle" && argc == 2 + PINOCHLE_PLAYER_NUM) || 
        (gameName == "HoldEm" && argc >= 2 + HOLDEM_PLAYER_NUM_MIN && argc <= 2 + HOLDEM_PLAYER_NUM_MAX)) {
        shared_ptr<Game> spGame = create(argc, argv);
        if (spGame) {
            return spGame -> play();
        }
        else {
            std::cout << "Creat game object failed." << std::endl;
            return CREATE_GAME_OBJECT_ERROR;
        }
    }
    else {
        std::cout << "Usage: ./lab1 Pinochle/HoldEm player1 player2 ..." << std::endl;
